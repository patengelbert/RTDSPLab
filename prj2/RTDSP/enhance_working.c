...
#define RESIDUALLEN 3				  /* Length of past buffer for residual noise removal*/
...
//#define SPECTRAL_POWER	// Perform spectral subtraction in power domain rather than frequency domain
//#define OVERSUBTRACTION	// Apply oversubtraction when output has low SNR
//define RESIDUAL_REMOVAL	// Perform risidual noise reduction
...
float *inFiltered, *noiseFiltered;	/* Filtered input and noise*/
complex *pastOutput[RESIDUALLEN];	/* Storage for past outputs*/
...
volatile int pastPtr = 0;			/* Past storage pointer*/
float filterConst, remFilterConst;	/* precomputed constants for filter*/
unsigned overhead, start, stop, sectionFFT,\
			sectionIFFT, sectionFilter, sectionMin, sectionSub, sectionRes; /* Timings*/
FILE *fp;							/* Log file */
...
complex cmin(complex a, complex b);						/* find minimum between two complex values*/
...
void main()
{
	...
	fprintf(fp, "Noise Frame Index\t|Cycle FFT\t|Cycle Filter\t|Cycle Min\t|Cycle Sub\t|Cycle Res\t|Cycle IFFT\n");
	fclose(fp);
#endif
...
void processFrame(void)
{
...
#if defined(BENCHMARK)
	stop = TIMER_getCount(timer);
	sectionFFT = min(stop - start - overhead, 0);
#endif
	...
	else
	{
		...
		for (k = 0; k < FFTLEN; k++)
		{
			...
			for (m = 0; m < FFTLEN; m++)
			{
				// Store the curernt output in the circular buffer 
				pastOutput[pastPtr][m] = cBuffer[m];
				// Find the minimum output within the buffer
				cBuffer[m] = cmin(pastOutput[0][m], pastOutput[1][m]);
				cBuffer[m] = cmin(cBuffer[m], pastOutput[2][m]);
			}
			if (++pastPtr >= RESIDUALLEN) pastPtr = 0; // Move to next buffer location

#if defined(BENCHMARK)
			stop = TIMER_getCount(timer);
			sectionRes = stop - start - overhead;
#endif
	...
	fprintf(fp, "%d\t\t\t|%d\t\t|%d\t\t|%d\t\t|%d\t\t|%d\t\t|%d\n", frameIndex, sectionFFT, sectionFilter, sectionMin, sectionSub, sectionRes, sectionIFFT);
	fclose(fp);
#endif
...
#if defined(SPECTRAL_POWER)
	// Calculate alpha^2*|N(m)|^2/|X(m)|^2
	attenuation = sqr(alphaValue*noiseFiltered[m]) / sqr(inFiltered[m]);
	// Calculate Y(m) = X(m)*G(m), where G(m)=max{lambda,sqrt(1-alpha*(|N(m)^2|/|X(m)|^2))}
	cBuffer[m] = rmul(max(MINFREQ, sqrtf(1 - attenuation), cBuffer[m]);
#else
	// Calculate |N(m)|/|X(m)|
	attenuation = alphaValue*noiseFiltered[m] / inFiltered[m];
	// Calculate Y(m) = X(m)*G(m), where G(m)=max{lambda,1-alpha*(|N(m)|/|X(m)|)}
	cBuffer[m] = rmul(max(MINFREQ, 1 - attenuation), cBuffer[m]);
#endif
...
complex cmin(complex a, complex b)
{
