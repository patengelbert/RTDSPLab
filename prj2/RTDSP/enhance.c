...
void spectralSubtraction(int m)
{
	...
	// If using the filtered_noise optimization
#if defined(FILTERED_NOISE)
	// Estimate the noise using the currently calculated minimum spectrum over last 10s and the previous noise value
	noiseFiltered[m] = (remFilterConst*minMagnitude[m]) + (noiseFiltered[m]*filterConst);
#else
	// If not using filtered_noise optimization, estimate noise as minimum spectrum over last 10 secs
	noiseFiltered[m] = minMagnitude[m];
#endif
	// Calculate |N(m)|/|X(m)|
	tempValue = noiseFiltered[m]/inFiltered[m];
	// If using oversubtraction optimization
#if defined(OVERSUBTRACTION)
	...
	if(lowSNR[m] != 0)
		...
		alphaValue = ALPHA;
	// If not using oversubtraction optimization, use the defined alpha value
#else
	alphaValue = ALPHA;
#endif
	// Calculate Y(m) = X(m)*G(m), where G(m)=max{lambda,1-alpha*(|N(m)|/|X(m)|)}
	cBuffer[m] = rmul(max(LAMBDA, 1-(alphaValue*tempValue)), cBuffer[m]);
	// If using oversubtraction optimization, recalculate SNR
#if defined(OVERSUBTRACTION)
