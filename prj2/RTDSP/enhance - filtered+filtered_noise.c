...
for(k=0;k<FFTLEN;k++)
	{
		...
		inFiltered[k] = sqrt(((1-filterConst)*pow(cabs(cBuffer[k]), 2)) + (pow(inFiltered[k],2)*filterConst));
#else
	...
	for(m=0; m <FFTLEN; m++)
	{
		...
		// TODO gt correct filtrConst (currently using same as input)
		noiseFiltered[m] = ((1-filterConst)*minMagnitude[m]) + (noiseFiltered[m]*filterConst);
		tempValue = noiseFiltered[m]/inFiltered[m];
#else
		tempValue = minMagnitude[m]/inFiltered[m];
#endif

#if defined(OVERSUBTRACTION)
		alphaValue = ALPHA*clamp(-log(m/2*FFTLEN)+1, MINOVERSUBTRACTION, MAXOVERSUBTRACTION);
#else
		alphaValue = ALPHA;
#endif
