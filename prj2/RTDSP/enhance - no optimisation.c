...
complex *cBuffer, *oldCBuffer;					/* Buffers for calculation */
float *magnitude[OVERSAMP], *minMagnitude, *inFiltered, *oldSampBuffer[ADJACENT];		/* Buffers to store minimum noise */
...
void processFrame(void)
{
	...
	// Update the frame index and move onto th next noise block if necessary
	frameIndex++;
...
#ifdef NOISE_AVARAGE_ENABLE
	oldSampBuffer[oldSampPtr] = inFiltered;
	/* retreive the spectrum of the previous sample (fabs required to make sure that index stays in range)*/
	inFiltered = oldSampBuffer[abs(oldSampPtr-1)%ADJACENT];
	// Swap to previous frame to calculate output
	tempCBuffer = oldCBuffer;
	oldCBuffer = cBuffer;
	cBuffer = tempCBuffer;
#endif
	...
	for (k=0;k<FFTLEN;k++)
	{
		temp = minMagnitude[k]/inFiltered[k];
		
#ifdef NOISE_AVARAGE_ENABLE
		if (temp > RESIDUAL_THRESHOLD)
		{
			// Calculate the minimum magnitude of the thre adjacent samples
			temp = min(minMagnitude[k]/inFiltered[k],minMagnitude[k]/oldSampBuffer[oldSampPtr][k]);
			temp = min(temp,minMagnitude[k]/oldSampBuffer[abs(oldSampPtr-2)%ADJACENT][k]);
		}
#endif
		cBuffer[k] = rmul(max(LAMBDA, 1-(ALPHA*temp)), cBuffer[k]);
	}
	
#ifdef NOISE_AVARAGE_ENABLE
	if (++oldSampPtr >= ADJACENT) oldSampPtr=0;
#endif
